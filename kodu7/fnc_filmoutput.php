...
function readpersonsinfilm($sortby, $sortorder){
	...
	if($sortby == 4){
		if($sordorder == 1){
			...
			$stmt = $conn->prepare("SELECT first_name, last_name, role, title FROM person JOIN person_in_movie ON person.person_id = person_in_movie.person_id JOIN movie ON movie.movie_id = person_in_movie.movie_id ORDER BY title DREF");
		}
	}
